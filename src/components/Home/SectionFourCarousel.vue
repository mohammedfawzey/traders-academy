<template>
  <section id="carousel" class="pt-12">
    <v-container fluid>
      <div
        class="container--content d-lg-flex align-center justify-space-around"
      >
        <v-btn
        ...
        >
          <v-icon :color="!prevDisabled ? 'grey darken-1' : ''"
            >{{$ar?'mdi-arrow-right':'mdi-arrow-left'}}</v-icon
          >
        </v-btn>
        ...
        >
          <v-icon :color="!nextDisabled ? 'grey darken-1' : ''"
            >{{$ar?'mdi-arrow-left':'mdi-arrow-right'}}</v-icon
          >
        </v-btn>
      </div>
      <div
...
export default {
  ...
  computed: {
    ...
    C_items() {
      return this.$t("section4Carousel");
    },
  },
  components: {
    CarouselItem,
